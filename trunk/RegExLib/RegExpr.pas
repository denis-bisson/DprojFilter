...
begin
  Result := nil;
  while (Str^ <> #0) and (Str^ <> Chr) do Inc(Str);
  if (Str^ <> #0) then Result := Str;
...
begin
  ...
  for i := 1 to Len do
    if ARegExpr[i] = '(' then inc(StackSz);
  ...
  try
      ...
      case ARegExpr[i] of
        '(':
          begin
            ...
            begin
              ...
              inc(i, 2); // skip '(?'
              i0 := i;
              while (i <= Len) and (ARegExpr[i] <> ')') do inc(i);
              if i > Len then
                Result := -1 // unbalansed '('
        ...
        '[':
          begin
            ...
            i0 := i;
            inc(i);
            if ARegExpr[i] = ']' {// cannot be 'emty' ranges - this interpretes} then inc(i); // as ']' by itself
            while (i <= Len) and (ARegExpr[i] <> ']') do
              if ARegExpr[i] = EscChar {//###0.942} then
                inc(i, 2) // skip 'escaped' char to prevent stopping at '\]'
              else
                inc(i);
            if (i > Len) or (ARegExpr[i] <> ']') {//###0.942} then Result := -(i0 + 1); // unbalansed '[' //###0.942
          end;
        '#': if AExtendedSyntax then
          begin
            // skip eXtended comments
            while (i <= Len) and (ARegExpr[i] <> #$D) and (ARegExpr[i] <> #$A)
              {// do not use [#$d, #$a] due to UniCode compatibility} do inc(i);
            while (i + 1 <= Len) and ((ARegExpr[i + 1] = #$D) or (ARegExpr[i + 1] = #$A)) do inc(i); // attempt to work with different kinds of line separators
    ...
    if StackIdx <> 0 then Result := -1; // unbalansed '('
    ...
    if (ASubExprs.Count = 0)
      ...
      {// well, we add it now:} then ASubExprs.InsertObject(0, ARegExpr, TObject((Len shl 16) or 1));
...
begin
  if programm <> nil then FreeMem(programm);
  if fExpression <> nil then FreeMem(fExpression);
  if fInputString <> nil then FreeMem(fInputString);
...
{$IFEND}
  begin
    ...
    if Result = Ch then Result := {$IFDEF FPC}AnsiLowerCase(Ch)[1]{$ELSE}REChar(CharLower(PChar(Ch))){$ENDIF};
  end;
...
begin
  ...
  begin
    ...
    while (Result > 0) and ((startp[Result] = nil)
      or (endp[Result] = nil)) do dec(Result);
  end
  else
    Result := -1;
...
begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned(fInputString)
    ...
    {//then Result := copy (fInputString, MatchPos [Idx], MatchLen [Idx]) //###0.929} then
...
begin
  ...
  if Result[length(Result)] = '-' {// remove '-' if all modifiers are 'On'} then System.Delete(Result, length(Result), 1);
...
{$IFNDEF D101}
  ...
  if Mask = succ(Mask) then sleep(1); //To eliminate  warning about not used value that he wants absolutely to initialize...
{$ENDIF}
...
begin
  if not ParseModifiersStr(AModifiers, fModifiers) then Error(reeModifierUnsupported);
...
begin
  ...
  // check modifiers
  if fModifiers <> fProgModifiers {//###0.941} then InvalidateProgramm;

  // can we optimize line separators by using sets?
{$IFNDEF UniCode}
  fLineSeparatorsSet := [];
  for i := 1 to length(fLineSeparators) do System.Include(fLineSeparatorsSet, fLineSeparators[i]);
{$ENDIF}

  // [Re]compile if needed
  if programm = nil then Compile; //###0.941

  // check [re]compiled programm
  if programm = nil then
    EXIT // error was set/raised by Compile (was reeExecAfterCompErr)
  else if programm[0] <> MAGIC {// Program corrupted.} then
...
begin
  ...
  if (p = nil) or (p = @regdummy) or (PREOp(p)^ <> BRANCH) then EXIT;
...
{$ENDIF}
    ...
    Result := true;
      ...
      if not Result then InvalidateProgramm;
      regexpbeg := nil;
...
begin
  ...
  begin
    ...
    if chain = nil {// First piece.} then
      flagp := flagp or flags and SPSTART
    else
      Tail(chain, latest);
    chain := latest;
  end;
  if chain = nil {// Loop ran zero times.} then EmitNode(NOTHING);
  Result := ret;
  ...
  case op of
    '*':
      begin
        ...
        if NonGreedyCh {//###0.940} then inc(regparse); // Skip extra char ('?')
      end; { of case '*'}
    '+':
      begin
    ...
    '{':
      begin
        ...
        inc(regparse);
        p := regparse;
        while Pos(regparse^, '0123456789') > 0 {// <min> MUST appear} do inc(regparse);
        ...
        begin
          inc(regparse);
          p := regparse;
          while Pos(regparse^, '0123456789') > 0 do inc(regparse);
          if regparse^ <> '}' then
          begin
            regparse := savedparse;
            EXIT;
        ...
        if BracesMin > 0 then flagp := WORST;
        if BracesMax > 0 then flagp := flagp or HASWIDTH or SPSTART;
...
{$ENDIF}
  ...
  var
    i: integer;
  begin
    for i := 1 to length(s) do EmitC(s[i]);
  end;

  function HexDig(ch: REChar): integer;
  begin
    Result := 0;
    if (ch >= 'a') and (ch <= 'f') then ch := REChar(ord(ch) - (ord('a') - ord('A')));
...
{$IFDEF UseSetOfChar}
  ...
  begin
    if b in RangeSet then EXIT;
    inc(RangeLen);
    if b < RangeChMin then RangeChMin := b;
    if b > RangeChMax then RangeChMax := b;
    Include(RangeSet, b);
  end;
{$ENDIF}
...
{$IFDEF UseSetOfChar}
    ...
    begin
      ...
      if RangeIsCI then EmitRangeCPrim(InvertCase(b)); //###0.930
    end
    else
    begin
{$IFDEF UseAsserts}
...
{$ENDIF}
      ...
      begin
        ...
        if (RangePCodeBeg - REOpSz - RENextOffSz)^ = ANYBUTTINYSET then RangeSet := [#0..#255] - RangeSet;
...
{$ENDIF}
  ...
  var
    i: integer;
  begin
    for i := 1 to length(s) do EmitRangeC(s[i]);
  end;
...
begin
  ...
  case (regparse - 1)^ of
    '^': if ((fCompModifiers and MaskModM) = 0)
        or ((fLineSeparators = '') and not fLinePairedSeparatorAssigned) then
        ret := EmitNode(BOL)
      else
        ret := EmitNode(BOLML);
    '$': if ((fCompModifiers and MaskModM) = 0)
...
{$IFDEF UniCode} //###0.935
              if (ord((regparse + 1)^) < 256)
                and (CharInSet(char((regparse + 1)^), ['d', 'D', 's', 'S', 'w', 'W'])) then
              begin
{$ELSE}
      ...
      begin
        ...
        begin
          ...
          begin // (?#comment)
            ...
            while (regparse^ <> #0) and (regparse^ <> ')') do inc(regparse);
            if regparse^ <> ')' then
            begin
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF})) then
      begin //###0.941 \x
        ...
        begin // Skip eXtended comment
          ...
          while (regparse^ <> #0) and (regparse^ <> #$D) and (regparse^ <> #$A) do inc(regparse);
          while (regparse^ = #$D) or (regparse^ = #$A) {// skip comment terminator} do inc(regparse); // attempt to support different type of line separators
        end
        else
        begin // Skip the blanks!
          while {$IFDEF UniCode}StrScan(XIgnoredChars, regparse^) <> nil //###0.947
{$ELSE}regparse^ in XIgnoredChars{$ENDIF} do inc(regparse);
        end;
        ret := EmitNode(COMMENT); // comment
      end
      else
      begin
        ...
        ender := (regparse + len)^;
        if (len > 1)
          and ((ender = '*') or (ender = '+') or (ender = '?') or (ender = '{')) then dec(len); // Back off clear of ?+*{ operand.
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF}) then EmitC(regparse^);
          inc(regparse);
          dec(len);
        end;
        EmitC(#0);
...
begin
  Result := 0;
  if (regexpbeg = nil) or (regparse = nil) then EXIT; // not in compiling mode ?
...
begin
  while (s^ <> #0) and (s^ <> ch) and (s^ <> InvertCase(ch)) do inc(s);
  if s^ <> #0 then
    Result := s
  else
    Result := nil;
...
begin
  ...
  if TheMax > AMax then TheMax := AMax;
  case PREOp(p)^ of
    ANY:
      begin
    ...
    BSUBEXP:
      begin //###0.936
        ...
        if sestart = nil then EXIT;
        seend := endp[ord(opnd^)];
        if seend = nil then EXIT;
        repeat
          opnd := sestart;
          while opnd < seend do
          begin
            if (scan >= fInputEnd) or (scan^ <> opnd^) then EXIT;
            inc(scan);
            inc(opnd);
          end;
          inc(Result);
        ...
        repeat
          ...
          begin
            if (scan >= fInputEnd) or
              ((scan^ <> opnd^) and (scan^ <> InvertCase(opnd^))) then EXIT;
            inc(scan);
            inc(opnd);
          end;
          inc(Result);
    ...
    case scan^ of
      ...
      BOUND:
        if (scan^ = BOUND)
          ...
          and ((reginput^ = #0) or (Pos(reginput^, fWordChars) <= 0))) then EXIT;

      BOL: if reginput <> fInputStart then EXIT;
      EOL: if reginput^ <> #0 then EXIT;
      BOLML: if reginput > fInputStart then
        begin
          nextch := (reginput - 1)^;
          if (nextch <> fLinePairedSeparatorTail)
            or ((reginput - 1) <= fInputStart)
            or ((reginput - 2)^ <> fLinePairedSeparatorHead) then
          begin
            if (nextch = fLinePairedSeparatorHead)
              and (reginput^ = fLinePairedSeparatorTail) then EXIT; // don't stop between paired separator
            if
{$IFNDEF UniCode}
...
{$ENDIF} then EXIT;
      ...
      EOLML: if reginput^ <> #0 then
        begin
          nextch := reginput^;
          if (nextch <> fLinePairedSeparatorHead)
            or ((reginput + 1)^ <> fLinePairedSeparatorTail) then
          begin
            if (nextch = fLinePairedSeparatorTail)
              and (reginput > fInputStart)
              and ((reginput - 1)^ = fLinePairedSeparatorHead) then EXIT; // don't stop between paired separator
            if
{$IFNDEF UniCode}
              not (nextch in fLineSeparatorsSet)
{$ELSE}
...
{$ELSE}(pos(reginput^, fLineSeparators) > 0){$ENDIF} then EXIT;
          inc(reginput);
        end;
      ANYDIGIT:
        begin
          if (reginput^ = #0) or (reginput^ < '0') or (reginput^ > '9') then EXIT;
          inc(reginput);
        end;
      NOTDIGIT:
        begin
          if (reginput^ = #0) or ((reginput^ >= '0') and (reginput^ <= '9')) then EXIT;
          inc(reginput);
        end;
{$IFNDEF UseSetOfChar} //###0.929
      ANYLETTER:
        begin
          if (reginput^ = #0) or (Pos(reginput^, fWordChars) <= 0) {//###0.943} then EXIT;
          inc(reginput);
        end;
      NOTLETTER:
        begin
      ...
      ANYSPACE:
        begin
          if (reginput^ = #0) or not (Pos(reginput^, fSpaceChars) > 0) {//###0.943} then EXIT;
          inc(reginput);
        end;
      NOTSPACE:
        begin
          if (reginput^ = #0) or (Pos(reginput^, fSpaceChars) > 0) {//###0.943} then EXIT;
          inc(reginput);
        end;
{$ENDIF}
      EXACTLYCI:
        begin
          ...
          if (opnd^ <> reginput^)
            and (InvertCase(opnd^) <> reginput^) then EXIT;
          len := strlen(opnd);
          //###0.929 begin
          no := len;
          save := reginput;
          ...
          begin
            ...
            inc(opnd);
            if (opnd^ <> save^)
              and (InvertCase(opnd^) <> save^) then EXIT;
            dec(no);
          end;
          //###0.929 end
          inc(reginput, len);
      ...
      BSUBEXP:
        begin //###0.936
          ...
          if startp[no] = nil then EXIT;
          if endp[no] = nil then EXIT;
          save := reginput;
          opnd := startp[no];
          while opnd < endp[no] do
          begin
            if (save >= fInputEnd) or (save^ <> opnd^) then EXIT;
            inc(save);
            inc(opnd);
          end;
          reginput := save;
        ...
        begin //###0.936
          ...
          begin
            if (save >= fInputEnd) or
              ((save^ <> opnd^) and (save^ <> InvertCase(opnd^))) then EXIT;
            inc(save);
            inc(opnd);
          end;
          reginput := save;
      ...
      ANYOFTINYSET:
        begin
          if (reginput^ = #0) or //!!!TinySet
            ...
            and (reginput^ <> (scan + REOpSz + RENextOffSz + 2)^)) then EXIT;
          inc(reginput);
        end;
      ANYBUTTINYSET:
        begin
          if (reginput^ = #0) or //!!!TinySet
            ...
            or (reginput^ = (scan + REOpSz + RENextOffSz + 2)^) then EXIT;
          inc(reginput);
        end;
{$IFDEF UseSetOfChar} //###0.929
      ANYOFFULLSET:
        begin
          if (reginput^ = #0)
            or not (reginput^ in PSetOfREChar(scan + REOpSz + RENextOffSz)^) then EXIT;
          inc(reginput);
        end;
{$ELSE}
      ANYOF:
        begin
          if (reginput^ = #0) or (StrScan(scan + REOpSz + RENextOffSz, reginput^) = nil) then EXIT;
          inc(reginput);
        end;
      ANYBUT:
        begin
      ...
      ANYOFCI:
        begin
          if (reginput^ = #0) or (StrScanCI(scan + REOpSz + RENextOffSz, reginput^) = nil) then EXIT;
          inc(reginput);
        end;
      ANYBUTCI:
        begin
...
{$ENDIF}
      ...
      Succ(OPEN)..TREOp(Ord(OPEN) + NSUBEXP - 1):
        begin //###0.929
          ...
          if not Result {//###0.936} then startp[no] := save;
      ...
      Succ(CLOSE)..TREOp(Ord(CLOSE) + NSUBEXP - 1):
        begin //###0.929
          ...
          Result := MatchPrim(next);
          if not Result {//###0.936} then endp[no] := save;
        ...
        begin //###0.940
          ...
          begin // Min alredy matched - we can work
            ...
            begin
              ...
              Result := MatchPrim(next);
              if not Result then reginput := save;
              EXIT;
            end
            else
            begin
...
{$ENDIF}
      STAR, PLUS, BRACES, STARNG, PLUSNG, BRACESNG:
        begin
          ...
          nextch := #0;
          if next^ = EXACTLY then nextch := (next + REOpSz + RENextOffSz)^;
          ...
          if (scan^ = BRACES) or (scan^ = BRACESNG) then inc(opnd, 2 * REBracesArgSz);
...
begin
  ...
  begin
    ...
    case PREOp(scan)^ of
      ...
      BRANCH:
        begin
          if (PREOp(next)^ <> BRANCH) {// No choice.} then
            next := scan + REOpSz + RENextOffSz // Avoid recursion.
          else
          begin
            repeat
...
{$IFDEF ComplexBraces}
      ...
      LOOP, LOOPNG:
        begin //###0.940
          ...
          if min_cnt = 0 then FillFirstCharSet(next);
          EXIT;
        end;
{$ENDIF}
      STAR, STARNG: //###0.940
...
begin
  ...
  if not IsProgrammOk {//###0.929} then EXIT;
  ...
  if AOffset > (InputLen + 1) {// for matching empty string after last char.} then EXIT;
  ...
  begin
    ...
    repeat
      ...
      if s <> nil then
      begin
        if StrLComp(s, regmust, regmlen) = 0 then BREAK; // Found it.
        inc(s);
      end;
    until s = nil;
    if s = nil {// Not present.} then EXIT;
  end;
...
{$IFDEF UseFirstCharSet}
      if s^ in FirstCharSet then Result := RegMatch(s);
{$ELSE}
      Result := RegMatch(s);
{$ENDIF}
      if Result or (s^ = #0) {// Exit on a match or after testing the end-of-string.} then
        EXIT
      else
        ClearMatchs; //###0.949
      inc(s);
...
begin
  ...
  if endp[0] = startp[0] {//###0.929} then inc(Offset); // prevent infinite looping if empty string match r.e.
...
begin
  ...
  if not Assigned(fInputString) then GetMem(fInputString, (Len + 1) * SizeOf(REChar));
...
begin
  ...
  begin
    ...
    if n >= 0 then
    begin
      if (n < NSUBEXP) and Assigned(startp[n]) and Assigned(endp[n]) then inc(ResultLen, endp[n] - startp[n]);
    end
    else
    begin
  ...
  if Exec(AInputStr) then
    repeat
      Result := Result + System.Copy(AInputStr, PrevPos,
        MatchPos[0] - PrevPos);
      if AUseSubstitution {//###0.946} then
...
begin
  ...
  begin // While that wasn't END last time...
    ...
    next := regnext(s);
    if next = nil {// Next ptr.} then
      Result := Result + ' (0)'
    else
    begin
      if next > s {//###0.948 PWideChar subtraction workaround (see comments in Tail method for details)} then
        Diff := next - s
      else
        Diff := -(s - next);
...
{$ENDIF}
  ...
  if regstart <> #0 then Result := Result + 'start ' + regstart;
  if reganch <> #0 then Result := Result + 'anchored ';
  if regmust <> nil then Result := Result + 'must have ' + regmust;
{$IFDEF UseFirstCharSet} //###0.929
...
begin
  ...
  if AErrorID < 1000 {// compilation error ?} then
