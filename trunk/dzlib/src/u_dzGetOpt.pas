...
type
  ...
  public
    ...
    ///<summary> The commandline that was parsed </summary>
...
begin
  ...
  while FoundIdx < _Options.Count do begin
    ...
    if not OptionPassed(OptName.Name) then
    begin
    ...
    begin
      raise EOptionValue.CreateFmt(_('Option %s can be present just once per line.'), [OptName.Name])
    end;
  end;
end;
...
begin
  ...
  Params := TStringList.Create;
  try
    TCmdLineParser.Execute(FCmdLine, Options, Params);
    EvaluateCmdLine(Options, Params);
  finally
    FreeAndNil(Params);
    FreeAndNil(Options);
